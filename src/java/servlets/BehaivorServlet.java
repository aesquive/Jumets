 ...
 * @author alvarerr
 */
public class BehaivorServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String  xsl = "<xsl:stylesheet xmlns:xsl=\"http://www.w3.org/1999/XSL/Transform\" version=\"1.0\"><xsl:template match=\"//CLIENTE\"> "+
		"<xsl:value-of select=\"CLI_ID\"/>,<xsl:value-of select=\"CLI_SAP\"/>,<xsl:value-of select=\"CLASIFICACION_1\"/>," +
                "<xsl:value-of select=\"CLASIFICACION_2\"/>,<xsl:value-of select=\"CLASIFICACION_3\"/>,<xsl:value-of select=\"ACCION_1\"/>," +
                "<xsl:value-of select=\"ACCION_2\"/></xsl:template></xsl:stylesheet>";
             //   request.getContextPath();


                System.out.println(request.getRequestURL());
		System.out.println("Inicia BEHAIVOR SERVLET SOLICITUD");
...
//				    System.out.println("datos archivo:\nfileName="+item.getFieldName()+":\nName="+item.getName()+":\n");
        ...
        CalculadoraBWS bws = new CalculadoraBWS();

        System.out.println("-------------->");
    //    System.out.println(bws.CalculaB(valorArchivoBehavior));

        String strXMLStringToByteArray =  bws.CalculaB(valorArchivoBehavior);
            ...
            URL foourl = this.getClass().getResource("/transf.xsl");
