...
public class PrimaMantServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request,
        ...
        try {
            ...
            if (command.equalsIgnoreCase("Guardar")) {
                ...
                p.setDescripcionOcupacion(request.getParameter("descripcionOcupacion"));
                p.setUbicacionVivienda(request.getParameter("ubicacionVivienda"));
                ...
                String lista = sacarListaClientes(nombre);
                request.getSession().setAttribute("lista", lista);
                pagina = "./prima/primaindex.jsp";
    ...
    public void calcular(HttpServletRequest request,
        ...
        double estado_woe = ((Edo) lista.get((Integer.parseInt(p.getUbicacionVivienda())) - 1)).getEdoWoe();
        double recargo = ((Edo) lista.get((Integer.parseInt(p.getUbicacionVivienda())) - 1)).getEdoRcg();
        
        q = s.createQuery("from RngEda as eda where eda.calId='6' and eda.rngEdaLimInf<="
        ...
        q = s.createQuery("from IntPi as intPi where intPi.cal.calId='6' and intPi.intPiLimInf<="
    ...
    private String sacarListaClientes(String nombre) {
        List<Cli> clientes = new LinkedList<Cli>();
        Dao dao = new Dao();
        List<CliPpr> tabla = dao.getTabla(CliPpr.class);
        for (CliPpr cliente : tabla) {
            Cli cli = cliente.getCli();
            String nombreCl = cli.getCliNom() + " " + cli.getCliApePat() + " " + cli.getCliApeMat();
            if (nombreCl.toUpperCase().contains(nombre.toUpperCase())) {
                clientes.add(cli);
            }
        }
        return formarOptions(clientes);
    }
    
    private String formarOptions(List<Cli> clientes) {
        StringBuilder builder = new StringBuilder();
        for (Cli cl : clientes) {
            builder.append("<option title='" + cl.getCliApePat() + " "
                    + cl.getCliApeMat() + " " + cl.getCliNom()
                    + "' value=\"" + cl.getCliId() + "\">"
                    + cl.getCliApePat() + " " + cl.getCliApeMat() + " "
                    + cl.getCliNom() + "</option>");
        }
