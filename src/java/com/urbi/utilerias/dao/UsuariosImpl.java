...
public class UsuariosImpl implements UsuariosDao {
	...
	public static int autentificacion(String user, String pass) {
		...
		String sql = "SELECT COUNT(*) FROM USU WHERE USU_CLAVE='" + user
				+ "' AND USU_PASSWORD='" + pass + "' ";

		try {
			String[][] resultado = ConexionDS.ejecutarSQL(sql, conn);
			if (resultado != null && resultado.length > 0)
				return ("1".equals(resultado[0][0])) ? 1 : 0;
			return 0;
		} catch (final SQLException sqle) {
			sqle.printStackTrace();
			return 0;
	 ...
	 * @return
	 */
	public static Usuarios usuarioUrbi(String user) {
		Connection conn = getConexion();
		String sql = "SELECT USU_ID, USU_CLAVE, USU_NOMBRE, USU_PERFIL "
				+ "FROM USU WHERE USU_CLAVE='" + user + "'";
		Usuarios usuario = new Usuarios();
		try {
			String[][] resultado = ConexionDS.ejecutarSQL(sql, conn);
			if (resultado != null && resultado.length > 0) {
