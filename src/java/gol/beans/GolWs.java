...
public class GolWs {
	...
	public void golWs() {
		...
		int VCS_VIV = Integer.parseInt(gol.getValorVivienda())
				/ Integer.parseInt(gol.getIngresoMensual());
		...
		if (REG_BURO == 1) { // SI ES UNO ES HIT
			...
			double SCORE_HIT_FACTOR = EsHit(s, GENERO, FRECUENCIA_RELIGION,
			...
			if (SCORE_HIT_FACTOR >= 800){
				...
				if (SCORE_NO_HIT >= 800) {// SCORE BURO META
					double SCORE_BURO = CalculoBuroMeta(s, ACT, HIST, ANT, USO,
							PAGO);
					if (SCORE_BURO >= 800)
			...
			double SCORE_NO_HIT = EsNoHit(s, RELIGION, ESCOLARIDAD,
					...
					RENTA, SERVICIO_HOGAR, VALOR_VIVIENDA, NO_CREDITOS, I_PSICO);
			if (SCORE_NO_HIT >= 800)
	...
	private double EsHit(Session s, int GENERO, int FRECUENCIA_RELIGION,
		...
		q = s
				.createQuery("from RngResAnt as sex where sex.calId = '3' and (sex.rngResAntLimInf<"
						...
						+ VIVIENDA_ACTUAL + ")");
		lista = (List) q.list();
                System.out.println("lleva viviendo " + VIVIENDA_ACTUAL);
	...
	private double EsNoHit(Session s, int RELIGION, int ESCOLARIDAD,
		...
		double SCORE_NO_HIT = RELIGION_FACTOR + ESCOLARIDAD_FACTOR
                ...
                /*
                System.out.println(gol.getApellidoPaterno() + " " + gol.getApellidoMaterno() + " " + gol.getNombre() +




                        ","+ RELIGION_FACTOR + ", " +ESCOLARIDAD_FACTOR
				+", "+TRABAJO_ANTERIOR_FACTOR +", " + DINERO_EXTRA_FACTOR +", "
				+INGRESO_TOTAL_FACTOR + ","+GASTOS_FACTOR +","+CUBRIR_ALIMETOS_FACTOR+", "
				+ VESTIDO_FACTOR + ", "+SERVICIOS_VIVIENDA_FACTOR +", "+RENTA_FACTOR+", "
				+ SERVICIO_HOGAR_FACTOR + ", "+PRINCIPAL_PROBLEMA_FACTOR+", "
				+ SOLICITAR_PRESTAMO_FACTOR+", " + VCS_VIV_FACTOR + ", "+I_RENTA_FACTOR+", "
				+ VALOR_VIVIENDA_FACTOR +", "+ NO_CREDITOS_FACTOR+", " + I_PSICO_FACTOR +","+SCORE_NO_HIT
